...
| `src/simstack4/pah_dither.py` | `DitherScheme`, `TruthSpectrum` (the independent direct-integration path — deliberately NOT the fit's own kernel, so injection-recovery is a real check), `compute_pz_matrix`, `NoiseModel` + shared-source covariance, `simulate_dithered_fluxes`, the Fisher/sweep helpers. **⚠️ Same rule as `pah_spectrum.py` below: the repo-root `CLAUDE.md` Module Map row is the single source of truth for arguments, defaults, and gotchas.** Record changes there. |
| `src/simstack4/pah_spectrum.py` | `build_design_matrix`, `warm_continuum_kernel`, `solve_linear_amplitudes` (GLS), `PAHSpectrumModel` and its fit paths (`fit_lstsq` / `fit_shared` / `fit_evolving` / `fit_evolving_mcmc` / `fit_with_alpha` / `fit_mcmc` / `pseudo_spectrum`), `evolving_flux_decomposition`. **⚠️ Capabilities, defaults, and gotchas are NOT duplicated here — the `pah_spectrum.py` row of the Module Map in the repo-root `CLAUDE.md` is the single source of truth.** That file is always loaded, so its version is in context before any work starts; this row is not. Record changes THERE, never here. (Why: this row was frozen at 2026-07-03 while the module moved on through branch-12 — it still advertised the pre-branch-12 defaults a month later, and a stale copy of a landmine list is worse than no copy. Same applies to the units trap, the τ_sil upper-limit caveat, and the plateau rejection.) |
