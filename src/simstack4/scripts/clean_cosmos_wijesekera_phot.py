...
_BAND_WAVELENGTHS = {
    "cfht-u":    3709,
    "sc-ib427":  4270, "sc-ia484":  4847, "sc-ib505":  5050,
    "sc-ia527":  5270, "sc-ib574":  5740, "sc-ia624":  6240,
    "sc-ia679":  6790, "sc-ib709":  7090, "sc-ia738":  7380,
    "sc-ia767":  7670, "sc-ib827":  8270,
    "hsc-g":     4770, "hsc-r":     6175, "hsc-i":     7711,
    "hsc-z":     8898, "hsc-y":     9762,
    "hst-f814w": 8140,
    "f115w":    11534, "f150w":    15007,
}
...
def compute_photometric_beta_luv(
    ...
    if not available_bands:
        print("  No flux columns found with prefix "
    ...
    flux_matrix = np.column_stack([
        df[available_bands[b]["flux_col"]].values.astype(float)
        for b in band_names
    ])
    ...
    if has_errors:
        err_matrix = np.column_stack([
            df[available_bands[b]["err_col"]].values.astype(float)
            for b in band_names
        ])
    else:
        err_matrix = None
    ...
    with warnings.catch_warnings():
        ...
        dl_mids = {
            round(zm, 3): Planck18.luminosity_distance(zm).value
            for zm in z_mids
        }
    ...
    if np.any(valid):
        print(f"    β range: [{beta_out[valid].min():.2f}, "
              f"{beta_out[valid].max():.2f}], "
              f"median = {np.median(beta_out[valid]):.2f}")
        print(f"    log(L_UV) range: [{log_luv_out[valid].min():.1f}, "
...
def create_wijesekera_catalog(
    ...
    *_col : str or None
    ...
    z_mask = (df[z_col] > 0.01) & (df[z_col] <= 5.0) & np.isfinite(df[z_col])
    mask &= z_mask
    ...
    if nuv_col and r_col and j_col_rf:
        if all(c in df.columns for c in [nuv_col, r_col, j_col_rf]):
            ...
            print(f"  NUVrJ flag:           {mask.sum():>8,} sources, "
    ...
    print(f"  Mass-complete flag:   {n_complete:>8,} complete, "
    ...
    if sfr_col and sfr_col in df.columns:
        ...
        n_sb = (~sb_mask).sum()
        print(f"  Starburst flag:       {n_sb:>8,} starbursts flagged "
    ...
    _pop_labels = {0: "complete_sfg", 1: "incomplete_sfg",
                   2: "all_qt", 3: "all_sb"}
    print(f"\n  Population classes:")
    ...
    if exclude_classes:
        ...
        for cls in exclude_classes:
            ...
            if cls not in (2,):  # warn if excluding anything other than QT
                print(f"    ⚠ WARNING: excluding {label} may bias stacking "
    ...
    if has_luv and beta_uv is not None:
        ...
        print(f"  L_UV(1600) from L_NUV + (β+1) correction: "
    ...
    if "beta_uv" in df_sel.columns:
        ...
        if np.any(both):
            ...
            print(f"  β comparison (phot vs template): "
    ...
    if "log_l_uv" in df_sel.columns:
        ...
        if np.any(both):
            ...
            print(f"  L_UV comparison (phot vs template): "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exclude", type=int, nargs="*", default=[2],
    ...
    if args.output:
        ...
        output_path = base / "COSMOSWeb_wijesekera_phot.parquet"
