...
def run_cosmos_stacking_pipeline():
    ...
    config_path = (
        Path(__file__).parent.parent.parent.parent / "config" / "cosmos25_luv.toml"
    )

    if not config_path.exists():
        # Alternative paths to try
        alt_paths = [
            "config/cosmos25_luv.toml",  # If run from project root
            "../../../config/cosmos25_luv.toml",  # Relative to script location
            Path.cwd()
            / "config"
            / "cosmos25_luv.toml",  # From current working directory
        ]
        ...
        for alt_path in alt_paths:
            ...
            print("❌ Could not find cosmos25_luv.toml in any of these locations:")
            print(
                f"   • {Path(__file__).parent.parent.parent.parent / 'config' / 'cosmos25_luv.toml'}"
            )
