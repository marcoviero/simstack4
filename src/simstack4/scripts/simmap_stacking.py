...
def run_cosmos_stacking_pipeline():
    ...
    if not config_path.exists():
        ...
        alt_paths = [
            "config/simstack_test.toml",  # If run from project root
            "../../../config/simstack_test.toml",  # Relative to script location
            Path.cwd()
            / "config"
            / "simstack_test.toml",  # From current working directory
        ]
        ...
        for alt_path in alt_paths:
            ...
            print(
                f"   • {Path(__file__).parent.parent.parent.parent / 'config' / 'simstack_test.toml'}"
            )
        ...
        if wrapper.processed_results:
            ...
            if len(measured) > 0:
                ...
                if "population_id" in summary_df.columns:
                    ...
                    print(f"  Star-forming: {len(sf_detected)} detected ")
                    print(f"  Quiescent:    {len(q_detected)} detected ")
