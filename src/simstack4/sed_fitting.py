...
class _DesignMatrix:
    ...
    def fit_transform(self, properties):
        ...
        self.stds_ = {
            k: max(np.nanstd(properties[k]), 1e-10) for k in self.property_names
        }
        self._fitted = True
    ...
    def _build(self, properties, n):
        ...
        for name in self.property_names:
            std[name] = (np.asarray(properties[name]) - self.means_[name]) / self.stds_[
                name
            ]
...
class RegressionGreybodyFitter:
    ...
    def _predict_fluxes(self, theta, X, wavelengths_obs, redshifts):
        ...
        for i in range(N_pop):
            ...
            S[i] = self.gb.greybody_model(
                wav_rest, log10_A[i], T_rest[i], self.gb.beta_fixed
            )
        return S
    ...
    def _init_from_individual_fits(self, X, sed_results):
        ...
        for i, (pop_id, sed) in enumerate(sed_results.items()):
            if (
                sed.dust_temperature_rest_frame is not None
        ...
        logger.info(
            f"Regression init from {len(T_vals)} individual fits "
            f"(T={T_vals.min():.1f}–{T_vals.max():.1f}K)"
        )
        return np.concatenate([theta_T, theta_A])

    def _init_from_flux_ratios(self, X, wavelengths_obs, fluxes, errors, redshifts):
        ...
        for i in range(N_pop):
            ...
            for T_try in np.arange(15, 75, 2):
                model = self.gb.greybody_model(wav_rest, 0.0, T_try, self.gb.beta_fixed)
                valid = (e > 0) & good
                if not np.any(valid):
                    continue
        ...
        logger.info(
            f"Regression init from flux ratios "
            f"(T={T_est.min():.0f}–{T_est.max():.0f}K)"
        )
        return np.concatenate([theta_T, theta_A])

    def fit(
        self, wavelengths_obs, fluxes, errors, redshifts, properties, sed_results=None
    ):
        """
        ...
        logger.info(
            f"Regression greybody: {N_pop} pops × {N_bands} bands "
        ...
        chi2_init = self._cost(theta0, X, wavelengths_obs, fluxes, errors, redshifts)
        ...
        for i in range(N_pop):
            ...
            L_IR[i] = self.gb._integrate_LIR(
                log10_A[i], T_rest[i], self.gb.beta_fixed, redshifts[i], D_L
            )
        ...
        logger.info(
            f"Regression final χ²/dof = {chi2_final / max(n_dof, 1):.2f} "
        ...
        for j, col in enumerate(self.dm.col_names_):
            logger.info(f"  {col:<20} θ_T={theta_T[j]:+.4f}  " f"θ_A={theta_A[j]:+.4f}")
