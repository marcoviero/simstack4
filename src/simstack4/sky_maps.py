...
from .exceptions.simstack_exceptions import MapError, ValidationError
...
class MapData:
    ...
    def pixel_scale(self) -> float:
        ...
        try:
            return abs(self.wcs.wcs.cdelt[0]) * 3600  # Convert to arcsec
        except:
    ...
    def get_rms_noise(self, mask: Optional[np.ndarray] = None) -> float:
...
class SkyMaps:
    ...
    def load_maps(self) -> None:
        ...
        for map_name, map_config in self.config.items():
            try:
                ...
                raise MapError(f"Failed to load map {map_name}: {e}")
    ...
    def _load_single_map(self, map_name: str, map_config: MapConfig) -> None:
            ...
            if noise_path.exists() and noise_path != map_path:
                ...
                try:
                    with fits.open(noise_path) as noise_hdul:
                        noise_hdu = noise_hdul[0] if len(noise_hdul) == 1 else noise_hdul[1]
        ...
        map_data = MapData(
            ...
            beam_area=map_config.beam.area,
    ...
    def _convert_units(self, map_data: MapData) -> None:
        ...
        if 'MJy/sr' in map_data.units:
            # Convert MJy/sr to Jy/beam
            # Factor = beam_area_sr * 1e6
            sr_per_arcsec2 = (np.pi / (180 * 3600))**2
            beam_area_sr = map_data.beam_area * sr_per_arcsec2
            ...
            logger.debug(f"Converted {map_data.map_name} from MJy/sr to Jy/beam")
    ...
    def _validate_map_compatibility(self) -> None:
        ...
        for map_name, map_data in self.maps.items():
            ...
            if abs(map_data.shape[0] - ref_shape[0]) > 10 or abs(map_data.shape[1] - ref_shape[1]) > 10:
                logger.warning(f"Map {map_name} has significantly different shape: {map_data.shape} vs {ref_shape}")

            # Check pixel scales are similar
            try:
                if abs(map_data.pixel_scale - ref_map.pixel_scale) > 0.5:  # 0.5 arcsec tolerance
                    logger.warning(f"Map {map_name} has different pixel scale: {map_data.pixel_scale:.2f} vs {ref_map.pixel_scale:.2f}")
            except:
                pass

    def world_to_pixel(self, map_name: str, ra: np.ndarray, dec: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def convolve_with_psf(self, data: np.ndarray, map_name: str,
        ...
        if method == 'fft':
            return convolve_fft(data, kernel, boundary='wrap',
                              nan_treatment='interpolate', normalize_kernel=False)
        else:
            return convolve(data, kernel, boundary='extend',
                          nan_treatment='interpolate', normalize_kernel=False)
    ...
    def print_map_summary(self) -> None:
        ...
        print("Map Details:")
        print(f"{'Name':<15} {'λ(μm)':<8} {'Shape':<12} {'Beam(\")':<10} {'Pixel(\")':<10} {'RMS':<10}")
        print("-" * 80)
